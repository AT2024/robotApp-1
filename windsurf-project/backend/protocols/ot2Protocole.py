# the magic numbers should be able to change via the gui, and this:thorium_lct,sds_lct,generators_locations
NUM_OF_GENERATORS = 5
THORIUM_VOL = 6.6000000000
SDS_VOL = 1.0000000000
CUR = 2
...
def run(ctx):
    ...
    First_iteration(dispense_amount, home_point, p1, p2, sds, temp_point, thorium, thorium_vol_aspirate)
...
def Init_objects(ctx):
    tiprack = ctx.load_labware('opentrons_96_tiprack_20ul', '1')
    copyTiprack = ctx.load_labware('opentrons_96_tiprack_20ul', '11')
    p1 = ctx.load_instrument('p20_single_gen2', 'left',
                             tip_racks=[tiprack])
    p2 = ctx.load_instrument('p20_single_gen2', 'right',
                             tip_racks=[tiprack])
    return tiprack, copyTiprack, p1, p2


def createLctPoint():
    firs_home = Location(Point(hight_home_lct[0], hight_home_lct[1], hight_home_lct[2]), None)
    first_temp = Location(Point(hight_temp_lct[0], hight_temp_lct[1], hight_temp_lct[2]), None)
...
def calculate_vol(dispense_amount):
    ...
    if (all_th <= 20):
        # thorium_vol_aspirate = THORIUM_VOL * NUM_OF_GENERATORS
...
def First_iteration(dispense_amount, home_point, p1, p2, sds, temp_point, thorium, thorium_vol_aspirate):
